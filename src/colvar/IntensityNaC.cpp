...
class IntensityNaC : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit IntensityNaC(const ActionOptions&);
  ~IntensityNaC();
// active methods:
...
PLUMED_REGISTER_ACTION(IntensityNaC,"INTENSITYNAC")

void IntensityNaC::registerKeywords( Keywords& keys ){
...
IntensityNaC::IntensityNaC(const ActionOptions&ao):
...
IntensityNaC::~IntensityNaC(){
  delete nl;
}

void IntensityNaC::prepare(){
  if(nl->getStride()>0){
...
void IntensityNaC::calculate()
{
  ...
  for(unsigned int m=0;m<qhist;++m){
    ...
    q=4*pi*std::sin(theta[m]*pi/180)/lamda;
      ...
      fij_Na= 4.7626*exp(-3.2850*(q/(4*pi))*(q/(4*pi)))+3.1736*exp(-8.8422*(q/(4*pi))*(q/(4*pi)))+1.2674*exp(-0.3136*(q/(4*pi))*(q/(4*pi)))+1.1128*exp(-129.424*(q/(4*pi))*(q/(4*pi)))+0.676;
...
//      value[m]+=2*std::sin(q*distanceModulo)/(getNumberOfAtoms()*q*distanceModulo);        

       Vector derivsf;
//       derivsf-=2/q*((q*std::cos(q*distanceModulo)/distanceModulo)-(std::sin(q*distanceModulo)/(distanceModulo*distanceModulo)))*distance_versor/(getNumberOfAtoms());              
        derivsf=-2*maxr/(q*pi*distanceModulo*distanceModulo*distanceModulo)*(distanceModulo*(q*std::cos(q*distanceModulo)*std::sin(pi*distanceModulo/maxr)+pi/maxr*std::sin(q*distanceModulo)*std::cos(pi*distanceModulo/maxr))-2*std::sin(q*distanceModulo)*std::sin(pi*distanceModulo/maxr))*distance_versor/(getNumberOfAtoms());
       ...
       comm.Sum(sfVirial);
         ...
         structureF[m]=fij_Na*fij_Na*(1+value[m]);

         for(unsigned i=0;i<getNumberOfAtoms();++i) setAtomsDerivatives(valueSF[m],i,fij_Na*fij_Na*sfPrime[m][i]);
         valueSF[m]->set(structureF[m]);
         setBoxDerivatives  (valueSF[m],fij_Na*fij_Na*sfVirial[m]);
