...
class IntensityO_Test : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit IntensityO_Test(const ActionOptions&);
  ~IntensityO_Test();
// active methods:
...
PLUMED_REGISTER_ACTION(IntensityO_Test,"INTENSITYO_TEST")

void IntensityO_Test::registerKeywords( Keywords& keys ){
...
IntensityO_Test::IntensityO_Test(const ActionOptions&ao):
...
// neighbor list stuff
  ...
  unsigned nweights=0;
  weight.resize(qhist, 1.0);
  for(unsigned i=0; i<qhist; ++i) {
    if( !parseNumbered( "WEIGHT", i+1, weight[i] ) ) break;
    nweights++;
  }
  if( nweights==0 ) {
    parse("WEIGHT",weight[0]);
    for(unsigned i=1; i<qhist; ++i) {
      weight[i]=weight[0];
    }
    nweights = qhist;
  }
...
IntensityO_Test::~IntensityO_Test(){
  delete nl;
}

void IntensityO_Test::prepare(){
  if(nl->getStride()>0){
...
// calculator
void IntensityO_Test::calculate()
{
  ...
  std::vector<Vector> sfPrime(getNumberOfAtoms());
  Tensor sfVirial;
...
//          value[m]+=2*std::sin(q*distanceModulo)/(getNumberOfAtoms()*q*distanceModulo)*std::sin(pi*distanceModulo/maxr)*maxr/(pi*distanceModulo);
     ...
     int q_num=distanceModulo/deltad;
      ...
      derivsf=prederivsf[q_num]*distance_versor;
       ...
       sfPrime[i0]+= derivsf*weight[m];
       sfPrime[i1]+= -derivsf*weight[m];
       Tensor vv(derivsf*weight[m], distance);
        sfVirial += vv ;
//   }
   }
       comm.Sum(value);
       ...
       comm.Sum(sfVirial);
         ...
         structureF+=fij_O*fij_O*(1+value[m])*weight[m];
//  log.printf(" fij_O %f \n",fij_O);

  }
  for(unsigned i=0;i<getNumberOfAtoms();++i) setAtomsDerivatives(i,fij_O*fij_O*sfPrime[i]);
  setValue(structureF);
  setBoxDerivatives  (fij_O*fij_O*sfVirial);
