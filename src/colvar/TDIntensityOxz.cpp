...
class TDIntensityOxz : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit TDIntensityOxz(const ActionOptions&);
  ~TDIntensityOxz();
// active methods:
...
PLUMED_REGISTER_ACTION(TDIntensityOxz,"TDINTENSITYOXZ")

void TDIntensityOxz::registerKeywords( Keywords& keys ){
...
TDIntensityOxz::TDIntensityOxz(const ActionOptions&ao):
...
TDIntensityOxz::~TDIntensityOxz(){
  delete nl;
}

void TDIntensityOxz::prepare(){
  if(nl->getStride()>0){
   ...
   for(unsigned int i=rank;i<nn;i+=stride) {
    ...
    if(pbc){
     ...
     distance=delta(getPosition(i0),getPosition(i1));
      ...
      d2=distance[0]*distance[0]+distance[2]*distance[2]; 
      distanceModulo=std::sqrt(d2);
      distance[1]=0.0;
      distance_versor = distance / distanceModulo;
      distance_versor[1]=0.0;
