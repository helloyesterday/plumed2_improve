...
class TDIntensityOyz : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit TDIntensityOyz(const ActionOptions&);
  ~TDIntensityOyz();
// active methods:
...
PLUMED_REGISTER_ACTION(TDIntensityOyz,"TDINTENSITYOYZ")

void TDIntensityOyz::registerKeywords( Keywords& keys ){
...
TDIntensityOyz::TDIntensityOyz(const ActionOptions&ao):
...
TDIntensityOyz::~TDIntensityOyz(){
  delete nl;
}

void TDIntensityOyz::prepare(){
  if(nl->getStride()>0){
   ...
   for(unsigned int i=rank;i<nn;i+=stride) {
    ...
    if(pbc){
     ...
     distance=delta(getPosition(i0),getPosition(i1));
      ...
      d2=distance[1]*distance[1]+distance[2]*distance[2]; 
      distanceModulo=std::sqrt(d2);
      distance[0]=0.0;
      distance_versor = distance / distanceModulo;
      distance_versor[0]=0.0;
