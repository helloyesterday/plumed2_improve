...
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
#ifndef __PLUMED_tools_NeighborList2DParallel_h
#define __PLUMED_tools_NeighborList2DParallel_h
...
public:
  NeighborList2DParallel(const std::vector<PLMD::AtomNumber>& list0,
               const std::vector<PLMD::AtomNumber>& list1,
               const bool& do_pair, const bool& do_pbc, const PLMD::Pbc& pbc, Communicator& cc,
               Log& log, const unsigned& _axis_id, const double& distance=1.0e+30,
               const double& _axis_dis=1e+30,const bool& do_full_list=false, const int& stride=0,
               const double& skin=0.1,const double& _axis_skin=0.1);
  NeighborList2DParallel(const std::vector<PLMD::AtomNumber>& list0, const bool& do_pbc,
               const PLMD::Pbc& pbc, Communicator& cc, Log& log, const unsigned& _axis_id,
               const double& distance=1.0e+30,const double& _axis_dis=1e+30,
               const bool& do_full_list=false, const int& stride=0, const double& skin=0.1, const double& _axis_skin=0.1);
